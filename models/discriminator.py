...
class PatchGANDiscriminator(nn.Module):
    ...
    Input: concat(noisy, clean_or_generated) = 6ch. Output: 14×14 patch scores."""

    def __init__(self, in_channels=6):
        super().__init__()

        def block(in_ch, out_ch, stride, use_bn):
            layers = [spectral_norm(nn.Conv2d(in_ch, out_ch, 4, stride=stride, padding=1, bias=not use_bn))]
            if use_bn:
        ...
        self.model = nn.Sequential(
            ...
            *block(256, 512, stride=1, use_bn=True),            # 16→15
            spectral_norm(nn.Conv2d(512, 1, 4, stride=1, padding=1)),  # 15→14
        )
